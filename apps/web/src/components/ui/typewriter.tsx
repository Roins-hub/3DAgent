...
const Typewriter = ({
  ...
  startFromFullText = false,
}: TypewriterProps) => {
  const initialText = Array.isArray(text) ? (text[0] ?? "") : text;
  const [displayText, setDisplayText] = useState(startFromFullText ? initialText : "");
  const [currentIndex, setCurrentIndex] = useState(startFromFullText ? initialText.length : 0);
